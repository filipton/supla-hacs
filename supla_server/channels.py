...
_KIND_ACTIONS: dict[str, list[str]] = {
    ...
    KIND_HVAC: ["off", "heat", "cool", "auto", "setpoint", "weekly_schedule", "turn_on"],
...
def _encode_thermostat_heatpol(
    ...
    return struct.pack("<BBhh", is_on, 0, 0, int(round(preset * 100)))
...
def decode_extended_value(ev_type: int, raw: bytes) -> dict[str, Any]:
    ...
    elif ev_type == C.EV_TYPE_IMPULSE_COUNTER_DETAILS_V1 and len(raw) >= 40:
